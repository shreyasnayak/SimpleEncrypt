...
int SimpleEncrypt::decrypt(unsigned char *ciphertext, int ciphertext_len, unsigned char *key,unsigned char *iv, unsigned char *plaintext)
{
    EVP_CIPHER_CTX *ctx;
    int len;
    int plaintext_len;
...
extern "C" {
    ...
    char* encryptMessage(char* message,char* ckey,char* civ)
    {
        ...
        /* Encrypt the data */
        ciphertext_len = SimpleEncrypt::encrypt((unsigned char *)message, static_cast<int>(strlen(message)),key, iv,ciphertext);
